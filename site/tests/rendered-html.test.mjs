...
test("server-renders the Ariadne product page and discovery metadata", async () => {
  ...
  assert.match(html, /New in v0\.8\.8/);
  assert.match(html, /Protected: remote Qdrant/);
  assert.match(html, /Scaled: metrics v3/);
...
test("server-renders the Ukrainian localized route", async () => {
  ...
  assert.match(html, /Нове у v0\.8\.8/);
  assert.match(html, /Захищено: remote Qdrant/);
...
test("keeps install, hosting, and social assets in the validated source", async () => {
  const [page, layout, hosting, og] = await Promise.all([
    ...
    access(new URL("../public/og-v0.8.8.png", import.meta.url)),
  ]);
  ...
  assert.match(layout, /og-v0\.8\.8\.png/);
