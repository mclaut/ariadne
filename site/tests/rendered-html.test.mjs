...
test("server-renders the Ariadne product page and discovery metadata", async () => {
  ...
  assert.match(html, /New in v0\.8\.12/);
  assert.match(html, /Bounded: scheduled maintenance/);
  assert.match(html, /Deterministic: curator output/);
...
test("server-renders the Ukrainian localized route", async () => {
  ...
  assert.match(html, /Нове у v0\.8\.12/);
  assert.match(html, /Обмежено: scheduled maintenance/);
...
test("keeps install, hosting, and social assets in the validated source", async () => {
  const [page, layout, hosting, og] = await Promise.all([
    ...
    access(new URL("../public/og-v0.8.12.png", import.meta.url)),
  ]);

  assert.match(page, /install\.ps1/);
  assert.match(page, /memory_move/);
  assert.match(page, /Local memory map/);
  assert.doesNotMatch(page, /SkeletonPreview|codex-preview/);
  assert.match(layout, /summary_large_image/);
  assert.match(layout, /1200/);
  assert.match(layout, /og-v0\.8\.12\.png/);
