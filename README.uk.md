...
- **macOS warning тепер виходить на передній план.** Нативний діалог спочатку
  активується, а вже потім показується, тому запит доступу не може непомітно
  залишитися позаду активного вікна Codex або Claude Code. Це змінює лише
  видимість: дозвіл усе одно з’являється виключно після явного рішення людини.
- **Hugging Face Space знову валідний.** `short_description` вкладається у ліміт
  Hub — не більше 60 символів.
