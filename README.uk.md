...
- **Розумний maintenance.** Окремий локальний quality gate відхиляє пам'ять,
  яка змішує незалежні теми або дублює той самий факт, після чого модель отримує
  одну цільову repair-спробу. Тимчасові network/Ollama помилки й далі мають
  bounded backoff, а детерміновані schema, language, local-path та quality
  помилки стають видимою відкладеною роботою без повтору всього stage.
