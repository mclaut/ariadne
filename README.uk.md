...
- **Оновлення macOS tray переживає його закриття.** Helper оновлення стартує в
  незалежній процесній сесії до завершення tray, тому завантажений інсталятор
  справді виконується, а не лишається непомітно невикористаним.
- **Кожна передача керування видима.** `update.log` фіксує старт helper до
  заміни tray, перетворюючи збій оновлення на діагностовану подію.
